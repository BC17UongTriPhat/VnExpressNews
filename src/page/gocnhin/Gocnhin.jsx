...
export default class Gocnhin extends Component {
  gocnhinList = [
    {
      id: 1,
  ...
  renderGocNhinDivBootStrap = () => {
    //output [<div className="col-4"></div>,<div className="col-4"></div>]
    return this.gocnhinList.map((gocnhin, index) => {
      return (
        <div  className="gocnhin_container container "  key={index}>
        <div  className="gocnhin_card row">
          <img className="col-6" style={{width:'237px', height:'144px', paddingLeft:'20px' }} src={gocnhin.img} alt="..." />
          <div className="gocnhin_cardbody col-6  ">
            <h3 style={{fontSize:'15px'}}>{gocnhin.title}</h3>
            <a href={gocnhin.url} >{gocnhin.content.length > 100 ? <span>{gocnhin.content.slice(0,108)}... <a  href={gocnhin.url}>Đọc tiếp</a> </span> :<span>{gocnhin.content}</span>}</a>
           <hr />
          </div>
        </div>
      </div>
    ...
    return (
      ...
      <div className="thoigioitabcontent tab-content " id="myTabContent">
        ...
        <div className="tab-pane fade show active " id="new" role="tabpanel" aria-labelledby="new-tab">
          <div>  {this.renderGocNhinDivBootStrap()}</div>
        </div>
        {/* Popular */}
